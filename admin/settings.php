...
class VI_WOO_COUPON_BOX_Admin_Settings {
    ...
    public function preview_emails_ajax() {
        if ( isset( $_GET['nonce'] ) && wp_verify_nonce( sanitize_key( wp_unslash( $_GET['nonce'] ) ), 'wcb_admin_nonce' ) ) {
    ...
    public function create_options_page() {
        add_submenu_page( 'edit.php?post_type=wcb', esc_html__( 'Settings', 'woo-coupon-box' ), esc_html__( 'Settings', 'woo-coupon-box' ), 'manage_options', 'woo_coupon_box', array( $this, 'setting_page_woo_coupon_box' ) );
    }
    ...
    public function setting_page_woo_coupon_box() {
        ...
        <div class="wrap">
            ...
            <p class="">
                <?php printf( "%s <a target='_blank' href='%s'>%s</a> %s", esc_html__( 'Please go to', 'woo-coupon-box' ), esc_url( admin_url( 'admin.php?page=wc-settings&tab=email#woocommerce_email_base_color' ) ), esc_html__( 'WooCommerce Emails settings', 'woo-coupon-box' ), esc_html__( 'to edit Email design', 'woo-coupon-box' ) ); ?>
            </p>
                                ...
                                <td>
                                    <div class="vi-ui toggle checkbox checked">
                                        ...
                                        <label for="wcb_coupon_unique_free_shipping">
                                            <span class="description">
                                                <?php
                                                printf( '%s <a href="https://docs.woocommerce.com/document/free-shipping/" target="_blank">%s</a> %s', esc_html__( 'Enable if the coupon grants free shipping. A free shipping method must be enabled in your shipping zone and be set to require "a valid free shipping coupon" (see the', 'woo-coupon-box' ), esc_html__( 'free shipping method', 'woo-coupon-box' ), esc_html__( 'setting).', 'woo-coupon-box' ) );
                                                ?>
                                            </span>
                                        </label>
                                    </div>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <p>{shop_now}
                                        ...
                                        printf( '<a class="wcb-button-shop-now" target="_blank" href="%s"  target="_blank" style="text-decoration:none;display:inline-block;padding:10px 30px;margin:10px 0;font-size:%spx;color:%s;background:%s;">%s</a>', esc_url( $url ), esc_attr( $this->settings->get_params( 'wcb_button_shop_now_size' ) ), esc_attr( $this->settings->get_params( 'wcb_button_shop_now_color' ) ), esc_attr( $this->settings->get_params( 'wcb_button_shop_now_bg_color' ) ), esc_attr( $this->settings->get_params( 'wcb_button_shop_now_title' ) ) );
                                        ?>
                                    </p>
                            ...
                            <tr valign="top">
                                ...
                                <td>
                                    <div>
                                        ...
                                        <ul>
                                            <li>
                                                <?php printf( '%s <a target="_blank" href="http://www.google.com/recaptcha/admin">%s</a> %s', esc_html__( '1, Visit', 'woo-coupon-box' ), esc_html__( 'page', 'woo-coupon-box' ), esc_html__( 'to sign up for an API key pair with your Gmail account', 'woo-coupon-box' ) ) ?>
                                            </li>
    ...
    public function admin_enqueue_script() {
        ...
        if ( $page == 'woo_coupon_box' ) {
            ...
            wp_enqueue_script( 'woo-coupon-box-form', VI_WOO_COUPON_BOX_JS . 'form.min.js', [ 'jquery' ], VI_WOO_COUPON_BOX_VERSION, true );
            wp_enqueue_script( 'woo-coupon-box-checkbox', VI_WOO_COUPON_BOX_JS . 'checkbox.min.js', [ 'jquery' ], VI_WOO_COUPON_BOX_VERSION, true );
            wp_enqueue_script( 'woo-coupon-box-dropdown', VI_WOO_COUPON_BOX_JS . 'dropdown.min.js', [ 'jquery' ], VI_WOO_COUPON_BOX_VERSION, true );
            wp_enqueue_script( 'woo-coupon-box-transition', VI_WOO_COUPON_BOX_JS . 'transition.min.js', [ 'jquery' ], VI_WOO_COUPON_BOX_VERSION, true );
            wp_enqueue_script( 'woo-coupon-box-tab', VI_WOO_COUPON_BOX_JS . 'tab.min.js', [ 'jquery' ], VI_WOO_COUPON_BOX_VERSION, true );
            wp_enqueue_script( 'woo-coupon-box-address', VI_WOO_COUPON_BOX_JS . 'address-1.6.min.js', [ 'jquery' ], VI_WOO_COUPON_BOX_VERSION, true );
            wp_enqueue_script( 'woo-coupon-box-select2', VI_WOO_COUPON_BOX_JS . 'select2.min.js', [ 'jquery' ], VI_WOO_COUPON_BOX_VERSION, true );
            wp_enqueue_script( 'woo-coupon-box-admin-javascript', VI_WOO_COUPON_BOX_JS . 'wcb-admin.js', [ 'jquery' ], VI_WOO_COUPON_BOX_VERSION, true );
            wp_localize_script( 'woo-coupon-box-admin-javascript', 'woo_coupon_box_params_admin', [ 'url' => admin_url( 'admin-ajax.php' ),  'nonce' => wp_create_nonce( 'wcb_admin_nonce' ) ] );
        }
    }
